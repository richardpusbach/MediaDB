...
3. Generate Prisma client:
   ...
   npm run db:generate
   ```
   This command now auto-checks Prisma package version mismatch and attempts a one-time package rebuild/retry if generation fails.
   If your output still shows `> prisma generate` (instead of `> node scripts/db-generate.mjs`), run `git pull` first.
   The Windows bootstrap script no longer depends on that npm script value and invokes `node scripts/db-generate.mjs` directly.
...
3. Regenerate Prisma client:
   ...
   npm run db:generate
   ```
   `db:generate` now performs extra self-healing steps (version check + rebuild + retry), so run it before manual reinstall attempts.
   You can verify your local script is updated with:
   ```powershell
   npm pkg get scripts.db:generate
   ```
   Expected value: `"node scripts/db-generate.mjs"`.

If `npm pkg get scripts.db:generate` still shows `"prisma generate"`, you are on stale files. Do this exactly:
```powershell
git pull
if (Test-Path .\node_modules) { Remove-Item .\node_modules -Recurse -Force }
del package-lock.json
