...
if (Test-Path "prisma/migrations") {
  Write-Host "Removing local prisma/migrations to ensure a clean dev init migration..." -ForegroundColor Yellow
...
try {
  ...
  if (Test-Path "prisma/migrations") {
    Write-Host "Removing prisma/migrations before retry..." -ForegroundColor Yellow
    Remove-Item "prisma/migrations" -Recurse -Force
  }

  Write-Host "Resetting public schema to clear failed migration history..." -ForegroundColor Yellow
  $resetSql = @"
