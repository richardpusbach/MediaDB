...
Write-Host "Verifying local db:generate script..." -ForegroundColor Yellow
$dbGenerateScript = npm pkg get scripts.db:generate
if ($dbGenerateScript -notmatch "db-generate.mjs") {
  Write-Host "Warning: scripts.db:generate does not point to db-generate.mjs. Continuing with direct node invocation." -ForegroundColor Yellow
}

Write-Host "Generating Prisma client..." -ForegroundColor Yellow
try {
  Invoke-Checked { node scripts/db-generate.mjs } "Prisma generate failed"
}
catch {
  ...
  Invoke-Checked { node scripts/db-generate.mjs } "Prisma generate failed after reinstall"
}
